...
func NewTableWithKeyRange(db *model.DBInfo, table *model.TableInfo) TableInfoWithKeyRange {
	// Use a non-keyspace-aware codec for backward compatibility with existing tests
	defaultCodec := tikv.NewCodecV1(tikv.ModeRaw)
	return newTableInfoWithKeyRange(db, table, nil, nil, defaultCodec)
}
...
func NewIndexWithKeyRange(db *model.DBInfo, table *model.TableInfo, index *model.IndexInfo) TableInfoWithKeyRange {
	// Use a non-keyspace-aware codec for backward compatibility with existing tests
	defaultCodec := tikv.NewCodecV1(tikv.ModeRaw)
	return newTableInfoWithKeyRange(db, table, nil, index, defaultCodec)
}
